...
CONTRIBUTING.md §2 says a change must update every file it touches. Remembering to do
...
def test_branch_layers_match_the_architecture():
    """Every branch-name layer must be a real part of the system, and vice versa."""
    guide = read("CONTRIBUTING.md")
    layers = set(re.findall(r"^\| `([a-z]+)` \|", guide, re.M))
...
def test_no_tooling_attribution_in_tracked_content():
    """House rule: the work is authored, not generated."""
    import subprocess

    banned = ("Co-Authored-By: Claude", "Claude-Session", "Generated with", "generated by Claude")
    ...
    for name in tracked:
        path = ROOT / name
        # This file names the banned phrases in order to search for them.
        if name == "tests/test_docs.py":
            continue
    ...
    assert not offenders, "tooling attribution in tracked files:\n" + "\n".join(offenders)
